<template>
    <div class="exhibitCont">
        <section class="itemCont" v-for="(item,index) in zlList" :key="index" @click="openInfo(item)">
            <div class="imgCont">
                <div class="covImg">
                    <img v-if="item.headPics && item.headPics != null" :src="item.pic[0]" alt="底图">
                ...
                <div class="footer">
                    ...
                    <span class="collectNum">收藏量{{ item.collectCount }}</span>
                    <section class="action">
                        <span class="downLow" @click.stop="upperOrDown(item.id,item.status)">{{ item.status == 1 ? '下架' : '上架' }}</span>
                        <span class="line"></span>
                        <span class="dele" @click.stop ="delTrumInfo(item.id)">删除</span>
                    </section>
                </div>
            </div>
...
import {ref,defineExpose,defineEmits,defineProps,onMounted} from 'vue';
import { useRouter } from 'vue-router';
const {proxy} = getCurrentInstance() as any;
const props = defineProps<{
    detailId:number
}>()
const zlList = ref<any>([
])
...
const getTrumlist = () => {
    proxy.API.getTrumlist({
        ...
        if(res.data.success){
            ...
            return {
              ...item,
              pic: item.headPics.split(",")
            }
          })
          pageTotal.value = res.data.result.totalItems
          zlList.value = newArray;
          console.log(zlList.value,'zlList.value')
        }
    })
}
const UpDateTrumInfo = (id:number,status:number) => {
    proxy.API.UpDateTrumInfo({
        status:status,
        id:id
    }).then((res:any) => {
        if(res.data.success){
...
const delTrumInfo = (id:number) => {
    proxy.API.delTrumInfo({id:id}).then((res:any)=> {
        if(res.data.success){
            getTrumlist()
