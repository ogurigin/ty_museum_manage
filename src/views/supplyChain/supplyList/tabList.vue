<template>
      <div class="handle-box">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item label="公司名称">
            <el-input v-model="formInline.name" placeholder="请输入公司名称" clearable />
          </el-form-item>
          <el-form-item>
      ...
      <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
        ...
        <el-table-column prop="pic" label="公司图片" align="center">
        ...
        <el-table-column prop="name" label="供应链类型" align="center">
          <template v-slot="scope">
            {{ scope.row.type == 1 ? "展览展陈" : "" }}
            {{ scope.row.type == 2 ? "展柜硬件" : "" }}
            {{ scope.row.type == 3 ? "多媒体视觉" : "" }}
            {{ scope.row.type == 4 ? "消防安防" : "" }}
            {{ scope.row.type == 5 ? "导视软装" : "" }}
          </template>
        </el-table-column>
...
const getData = () => {
  ...
  proxy.API.getSupplyList(params).then((res: any) => {
    ...
    pageTotal.value = res.data.result.totalItems || 0;
  }).catch((err: any) => {
    console.log(err);
  });
};
...
const delSupplyInfo = (id: number) => {
  let params = {
    id,
  }
  proxy.API.delSupplyInfo(params).then((res: any) => {
    ElMessage({
      type: 'success',
      message: '删除成功',
    });
